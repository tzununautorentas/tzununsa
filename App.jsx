...
// ─── ErrBoundary (inline para evitar problemas de ruta) ───────────────────────
...
function ModIcon({ mod, size = 52 }) {
  return (
    <div style={{
      ...
      letterSpacing:-0.5, boxShadow:`0 4px 12px ${mod.c1}55`, flexShrink:0,
    }}>
      {mod.letters}
    </div>
  );
...
function Toast({ toast }) {
  ...
  return (
    <div style={{
      ...
      {toast.type==="err" ? "Error" : "OK"} — {toast.msg}
    </div>
  );
}

// ─── Boton tema ───────────────────────────────────────────────────────────────
function BtnTema({ collapsed }) {
  return (
    <button onClick={toggleTheme}
      title={isDark ? "Cambiar a tema claro" : "Cambiar a tema oscuro"}
      style={{
        display:"flex", alignItems:"center", gap:8,
        background:"transparent", border:`1px solid ${T.bord}`,
        borderRadius:8, padding:collapsed?"8px":"7px 12px",
        cursor:"pointer", color:T.sub, fontSize:12,
        width:"100%", justifyContent:collapsed?"center":"flex-start",
      }}>
      <span>{isDark ? "Sol" : "Luna"}</span>
      {!collapsed && <span style={{ fontWeight:600 }}>{isDark ? "Tema claro" : "Tema oscuro"}</span>}
    </button>
  );
}

// ─── Menu completo (bottom sheet) ────────────────────────────────────────────
      ...
      <div style={{ position:"absolute", bottom:0, left:0, right:0, background:T.surf, borderRadius:"24px 24px 0 0", maxHeight:"85vh", overflowY:"auto" }}
        ...
        <div style={{ padding:"14px 18px 100px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
            ...
            <button onClick={toggleTheme}
              style={{ background:T.card, border:`1px solid ${T.bord}`, borderRadius:10, padding:"6px 14px", cursor:"pointer", color:T.sub, fontSize:12, fontWeight:600 }}>
...
// ─── Home movil ───────────────────────────────────────────────────────────────
function HomeMovil({ setPag, userName, empId }) {
  ...
  return (
    <div>
      ...
      <button onClick={()=>setPag("dashboard")}
        ...
        <div style={{ marginLeft:"auto", fontSize:20, color:T.acc }}>+</div>
      </button>
    </div>
  );
}
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:T.bg, color:T.txt, fontFamily:"'Inter','Segoe UI',sans-serif", overscrollBehavior:"none" }}>
      ...
      <div style={{ background:T.surf, padding:"14px 16px 12px", display:"flex", alignItems:"center", gap:12, flexShrink:0, borderBottom:`1px solid ${T.bord}` }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontSize:10, color:T.sub }}>{onHome?"Propietario":TITULOS[pag]||""}</div>
        </div>
        <button onClick={toggleTheme} title={isDark?"Tema claro":"Tema oscuro"}
          style={{ width:36, height:36, borderRadius:10, background:T.card, border:`1px solid ${T.bord}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700, color:T.acc, cursor:"pointer" }}>
          {isDark?"DIA":"NOC"}
        </button>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"16px 14px", paddingBottom:90 }}>
        {onHome
          ? <HomeMovil setPag={navegar} userName={userName} empId={empId}/>
          ...
          return (
            <button key={tab.id}
              ...
              <div style={{ width:36, height:36, borderRadius:10, background:isActive?color+"22":"transparent", display:"flex", alignItems:"center", justifyContent:"center" }}>
                {tab.id==="__menu__"
                  ? <span style={{ fontSize:18, color:T.mut, letterSpacing:2 }}>...</span>
    ...
    <div style={{ minHeight:"100vh", background:T.bg, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ width:"100%", maxWidth:380 }}>
        ...
        <form onSubmit={login} style={{ ...S.card, display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <button type="submit" disabled={loading} style={{ ...S.btn("primary"), padding:14, fontSize:14 }}>{loading?"Ingresando...":"Entrar"}</button>
        </form>
        <div style={{ textAlign:"center", marginTop:16, fontSize:11, color:T.mut }}>tzununsa.vercel.app</div>
        <div style={{ textAlign:"center", marginTop:10 }}>
          <button onClick={toggleTheme} style={{ background:"transparent", border:"none", color:T.mut, cursor:"pointer", fontSize:12 }}>
    ...
    <div style={{ display:"flex", minHeight:"100vh", background:T.bg, color:T.txt, fontFamily:"'Inter','Segoe UI',sans-serif" }}>
      ...
      <div style={{ width:collapsed?56:216, background:T.surf, borderRight:`1px solid ${T.bord}`, flexShrink:0, display:"flex", flexDirection:"column", transition:"width .2s", overflow:"hidden" }}>
        ...
        <div style={{ borderTop:`1px solid ${T.bord}`, padding:collapsed?"10px 4px":"10px 12px", display:"flex", flexDirection:"column", gap:6 }}>
          {!collapsed&&<div style={{ fontSize:10, color:T.sub, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{userName}</div>}
          <BtnTema collapsed={collapsed}/>
...
export default function App() {
  ...
  useEffect(()=>{
    if(session) dbGet("empresas","&select=id&limit=1").then(d=>{ if(d?.[0]) setEmpId(d[0].id); });
  },[session]);
