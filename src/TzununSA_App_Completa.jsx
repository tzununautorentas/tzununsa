...
export default function App(){
  ...
  const del=async id=>{if(!confirm("¿Eliminar esta cotización?"))return;await dbDel("cotizaciones",id);showToast("Eliminada");load();};
  const chEst=async(id,estado)=>{await dbUpd("cotizaciones",id,{estado,orden_venta:estado==="orden_venta"});showToast(`→ ${estado}`);load();};
  const clonar=r=>{setEditItem({...r,__clon:true});setVista("form");};
  const editar=r=>{setEditItem(r);setVista("form");};
  const filtered=filtro==="todas"?rows:rows.filter(r=>r.estado===filtro||((filtro==="orden_venta")&&r.orden_venta));

  if(vista==="form") return (
    <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",background:T.bg,color:T.txt,minHeight:"100vh",padding:22}}>
      {toast&&<Toast msg={toast.msg} type={toast.type}/>}
      <FormCotizacion initial={editItem} empId={empId} clientes={clientes}
        onSave={estado=>{showToast(estado==="orden_venta"?"Convertida a Orden de Venta ✔":editItem?.__clon?"Cotización clonada ✔":editItem?.id?"Cotización actualizada ✔":"Cotización guardada ✔");setEditItem(null);setVista("lista");load();}}
        onCancel={()=>{setEditItem(null);setVista("lista");}}/>
    </div>
  );

  return (
    <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",background:T.bg,color:T.txt,minHeight:"100vh"}}>
      {preview&&<ModalVistaPrevia cot={preview} onClose={()=>setPreview(null)}/>}
      <div style={{background:T.surf,borderBottom:`1px solid ${T.bord}`,padding:"12px 22px",display:"flex",alignItems:"center",gap:14}}>
        <div style={{width:36,height:36,borderRadius:9,background:"linear-gradient(135deg,#00D4AA,#3B82F6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>🐦</div>
        <div><div style={{fontSize:15,fontWeight:800,color:T.acc}}>Tz'unun AutoRentas</div><div style={{fontSize:10,color:T.mut}}>Módulo de Cotizaciones</div></div>
        <button onClick={()=>{setEditItem(null);setVista("form");}} style={{...S.btn("primary"),marginLeft:"auto"}}>+ Nueva cotización</button>
      </div>
      <div style={{padding:22}}>
        {toast&&<Toast msg={toast.msg} type={toast.type}/>}
        {/* Stats */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:18}}>
          {[{l:"Total",v:rows.length,c:T.acc},{l:"Enviadas",v:rows.filter(r=>r.estado==="enviada").length,c:T.blue},{l:"Aprobadas",v:rows.filter(r=>r.estado==="aprobada").length,c:T.acc},{l:"Órdenes de Venta",v:rows.filter(r=>r.orden_venta).length,c:"#A855F7"},{l:"Valor total",v:`Q ${fmt(rows.filter(r=>r.estado!=="rechazada").reduce((s,r)=>s+(parseFloat(r.total_gtq)||0),0)).split(".")[0]}`,c:T.acc}].map((s,i)=>(
            <div key={i} style={{background:T.surf,borderRadius:10,padding:14,textAlign:"center"}}>
              <div style={{fontSize:i===4?14:22,fontWeight:800,color:s.c}}>{s.v}</div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
          {["todas","borrador","enviada","aprobada","rechazada","orden_venta"].map(f=>(
            <button key={f} onClick={()=>setFiltro(f)} style={{...S.btn(filtro===f?"primary":"ghost"),fontSize:11,padding:"5px 12px"}}>
        ...
        {loading?<div style={{textAlign:"center",padding:40,color:T.sub}}>⏳ Cargando...</div>:
         filtered.length===0?<div style={{...S.card,textAlign:"center",padding:36,color:T.sub}}>📭 Sin cotizaciones</div>:(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {filtered.map(r=>{
              const e=r.orden_venta?EST.orden_venta:(EST[r.estado]||EST.borrador);
              const total=parseFloat(r.total_gtq)||0;
              const exch=parseFloat(r.tasa_cambio)||7.70;
              return (
                <div key={r.id} style={S.card}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                    <div>
                      ...
                      {r.saludo&&<div style={{fontSize:12,color:T.sub,fontStyle:"italic",marginTop:2}}>"{r.saludo}"</div>}
                      <div style={{fontSize:12,color:T.sub,marginTop:3}}>
                        {r.tipo==="renta"?"🔑 Renta":"🗺 Traslado"} · {r.dias} día{r.dias!==1?"s":""}
                        {r.vehiculo_nombre&&` · ${r.vehiculo_nombre}`}
                        {r.tasa_iva&&` · IVA ${r.tasa_iva}%`}
                      </div>
                      {r.descripcion_servicio&&<div style={{fontSize:11,color:T.sub,marginTop:2,fontStyle:"italic",maxWidth:400,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.descripcion_servicio}</div>}
                    </div>
                    <div style={{textAlign:"right"}}>
                      {r.orden_venta&&<div style={{fontSize:10,fontWeight:700,color:"#A855F7",marginBottom:4}}>📦 ORDEN DE VENTA</div>}
                      <div style={{marginBottom:4}}><span style={{display:"inline-block",padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:600,color:e.c,background:e.bg}}>{e.l}</span></div>
                      <div style={{fontSize:16,fontWeight:800,color:T.acc}}>Q {fmt(total)}</div>
                      <div style={{fontSize:11,color:T.sub}}>$ {fmt(total/exch)} USD</div>
                    </div>
                  </div>
                  {/* Acciones */}
                  <div style={{display:"flex",gap:6,paddingTop:10,borderTop:`1px solid ${T.bord}22`,flexWrap:"wrap"}}>
                    <button onClick={()=>setPreview(r)} style={{...S.btn("blue"),fontSize:11,padding:"5px 10px"}}>👁 Ver PDF</button>
                    <button onClick={()=>{const doc=generarPDF({numero:r.numero,fecha:r.fecha_emision||today(),fecha_vence:r.fecha_vence,cliente:r.cliente_nombre,nit:r.cliente_nit,dir_cliente:r.cliente_dir,saludo:r.saludo,servicio:r.descripcion_servicio,caract:r.caract||["Vehículo","Aire acond.","Cinturones","Seguro"],incluidos:r.incluidos||["Combustible","Conductor","Atención"],beneficios:r.beneficios||["Viaje seguro","Puntualidad","Flexibilidad"],con_piloto:r.con_piloto!==false,sub:parseFloat(r.subtotal)||0,iva_pct:parseFloat(r.tasa_iva)||5,iva_amt:parseFloat(r.total_iva)||0,total_ef:parseFloat(r.total_gtq)||0,total_tc:(parseFloat(r.total_gtq)||0)*1.05,exch:parseFloat(r.tasa_cambio)||7.70,es_orden:r.orden_venta});if(doc)doc.save(`${r.numero}.pdf`);}} style={{...S.btn("primary"),fontSize:11,padding:"5px 10px"}}>⬇ PDF</button>
                    <button onClick={()=>{const doc=generarPDF({numero:r.numero,fecha:r.fecha_emision||today(),fecha_vence:r.fecha_vence,cliente:r.cliente_nombre,nit:r.cliente_nit,dir_cliente:r.cliente_dir,saludo:r.saludo,servicio:r.descripcion_servicio,caract:r.caract||["Vehículo","Aire acond.","Cinturones","Seguro"],incluidos:r.incluidos||["Combustible","Conductor","Atención"],beneficios:r.beneficios||["Viaje seguro","Puntualidad","Flexibilidad"],con_piloto:r.con_piloto!==false,sub:parseFloat(r.subtotal)||0,iva_pct:parseFloat(r.tasa_iva)||5,iva_amt:parseFloat(r.total_iva)||0,total_ef:parseFloat(r.total_gtq)||0,total_tc:(parseFloat(r.total_gtq)||0)*1.05,exch:parseFloat(r.tasa_cambio)||7.70,es_orden:r.orden_venta});if(doc){const blob=doc.output("blob");const url=URL.createObjectURL(blob);const w=window.open(url);setTimeout(()=>w?.print(),1000);}}} style={{...S.btn("ghost"),fontSize:11,padding:"5px 10px"}}>🖨️ Imprimir</button>
                    <button onClick={()=>{const sub=encodeURIComponent(`Cotización ${r.numero} — Tz'unun AutoRentas`);const bod=encodeURIComponent(`Estimados,\n\nAdjunto cotización ${r.numero} por Q ${fmt(parseFloat(r.total_gtq)||0)}.\n\nSaludos,\nOscar Gálvez\nTz'unun AutoRentas\n502-31221538`);window.open(`mailto:?subject=${sub}&body=${bod}`);}} style={{...S.btn("ghost"),fontSize:11,padding:"5px 10px"}}>✉️ Email</button>
                    <button onClick={()=>clonar(r)} style={{...S.btn("ghost"),fontSize:11,padding:"5px 10px"}}>📋 Clonar</button>
                    <button onClick={()=>editar(r)} style={{...S.btn("ghost"),fontSize:11,padding:"5px 10px"}}>✏️ Editar</button>
                    {!r.orden_venta&&r.estado!=="rechazada"&&<button onClick={()=>chEst(r.id,"orden_venta")} style={{...S.btn("purple"),fontSize:11,padding:"5px 10px"}}>📦 → Orden</button>}
                    {r.estado==="enviada"&&<button onClick={()=>chEst(r.id,"aprobada")} style={{...S.btn("primary"),fontSize:11,padding:"5px 10px"}}>✅ Aprobar</button>}
                    <button onClick={()=>del(r.id)} style={{...S.btn("danger"),fontSize:11,padding:"5px 10px",marginLeft:"auto"}}>🗑️</button>
                  </div>
                </div>
              );
            })}
...
export default function App(){
  ...
  const [authFac,setAuthFac]=useState(null);
  ...
  useEffect(()=>{
    dbGet("empresas","&select=*&limit=1").then(d=>{if(d&&d[0])setEmpId(d[0].id);});
    dbGet("clientes","").then(d=>setClientes(Array.isArray(d)?d:[]));
    dbGet("reservas","").then(d=>setReservas(Array.isArray(d)?d:[]));
    dbGet("cotizaciones","&estado=eq.aprobada&orden=created_at.desc").then(d=>setCotizaciones(Array.isArray(d)?d:[]));
    dbGet("movimientos_bancarios","&tipo=eq.ingreso&order=fecha.desc").then(d=>setAnticipos(Array.isArray(d)?d:[]));
    load();
  },[]);

  const load=async()=>{setLoading(true);const d=await dbGet("facturas");setRows(Array.isArray(d)?d:[]);setLoading(false);};

  const anular=async(factura,motivo)=>{
    if(!motivo.trim()){showToast("Ingresa el motivo de anulación","err");return;}
    await dbUpd("facturas",factura.id,{estado:"anulada",motivo_anulacion:motivo});
    showToast("Factura anulada");setModalAnular(null);load();
  };

  const registrarPago=async(factura,monto,fecha,metodo)=>{
    const nuevoSaldo=Math.max(0,(parseFloat(factura.saldo_pendiente)||parseFloat(factura.total)||0)-monto);
    const nuevoEstado=nuevoSaldo<=0?"pagada":"parcial";
    await dbUpd("facturas",factura.id,{saldo_pendiente:nuevoSaldo,estado:nuevoEstado,fecha_pago:fecha});
    // Registrar en La Banca
    await dbIns("movimientos_bancarios",{
      empresa_id:empId,fecha,tipo:"ingreso",
      descripcion:`Pago factura ${factura.numero} — ${factura.nombre_receptor}`,
      monto,referencia:factura.numero,categoria:"ventas",
      factura_id:factura.id,conciliado:true,metodo_pago:metodo,
    });
    showToast(nuevoSaldo<=0?"Factura pagada completa ✔":"Pago parcial registrado ✔");
    setModalPago(null);load();
  };

  const registrarAuth=async()=>{
    if(!authId.trim()){showToast("Ingresa el número de autorización DTE","err");return;}
    await dbUpd("facturas",authFac.id,{numero_autorizacion:authId,estado:"certificada",fecha_certificacion:new Date().toISOString()});
    showToast("DTE certificado ✔");setAuthFac(null);setAuthId("");load();
  };

  const filtered=filtro==="todas"?rows:rows.filter(r=>r.estado===filtro);
  const totalFac=rows.filter(r=>!["anulada","borrador"].includes(r.estado)).reduce((s,r)=>s+(parseFloat(r.total)||0),0);
  const totalIva=rows.filter(r=>!["anulada","borrador"].includes(r.estado)).reduce((s,r)=>s+(parseFloat(r.monto_iva)||0),0);
  const totalSaldo=rows.filter(r=>!["anulada","pagada"].includes(r.estado)).reduce((s,r)=>s+(parseFloat(r.saldo_pendiente)||0),0);

  if(vista==="form") return (
    <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",background:T.bg,color:T.txt,minHeight:"100vh",padding:22}}>
      {toast&&<Toast msg={toast.msg} type={toast.type}/>}
      <FormFactura initial={editItem} empId={empId} clientes={clientes} reservas={reservas}
        cotizaciones={cotizaciones} anticipos={anticipos}
        onSave={()=>{showToast(editItem?.id?"Factura actualizada ✔":"Factura guardada ✔");setEditItem(null);setVista("lista");load();}}
        onCancel={()=>{setEditItem(null);setVista("lista");}}/>
    </div>
  );
  ...
  return (
    <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",background:T.bg,color:T.txt,minHeight:"100vh"}}>
      <ModalAnular factura={modalAnular} onConfirm={(m)=>anular(modalAnular,m)} onCancel={()=>setModalAnular(null)}/>
      <ModalPago factura={modalPago} onConfirm={(monto,fecha,metodo)=>registrarPago(modalPago,monto,fecha,metodo)} onCancel={()=>setModalPago(null)}/>

      {/* Auth DTE Modal */}
      {authFac&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:T.card,borderRadius:16,border:`1px solid ${T.acc}`,width:"100%",maxWidth:500,padding:24}}>
            <div style={{fontSize:14,fontWeight:700,color:T.acc,marginBottom:10}}>🔐 Registrar No. Autorización DTE</div>
            <div style={{fontSize:12,color:T.sub,marginBottom:14}}>{authFac.numero} · {authFac.nombre_receptor}</div>
            <label style={S.lbl}>NÚMERO DE AUTORIZACIÓN SAT (UUID)</label>
            <input style={{...S.inp,fontFamily:"monospace",marginBottom:14}} value={authId} onChange={e=>setAuthId(e.target.value)} placeholder="8f2a-4c1b-9e3d-7b5a..."/>
            <div style={{display:"flex",gap:8}}>
              <button onClick={registrarAuth} style={{...S.btn("primary"),flex:1}}>✅ Certificar</button>
              <button onClick={()=>{setAuthFac(null);setAuthId("");}} style={{...S.btn("ghost"),flex:1}}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{background:T.surf,borderBottom:`1px solid ${T.bord}`,padding:"12px 22px",display:"flex",alignItems:"center",gap:14}}>
        <div style={{width:36,height:36,borderRadius:9,background:"linear-gradient(135deg,#00D4AA,#3B82F6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>🐦</div>
        <div><div style={{fontSize:15,fontWeight:800,color:T.acc}}>Tz'unun AutoRentas</div><div style={{fontSize:10,color:T.mut}}>Facturación FEL — NIT Emisor: 16693949</div></div>
        <button onClick={()=>{setEditItem(null);setVista("form");}} style={{...S.btn("primary"),marginLeft:"auto"}}>+ Nueva factura</button>
      </div>
      ...
      <div style={{padding:22}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:18}}>
          {[
            {l:"Total facturas",v:rows.length,c:T.acc},
            {l:"Emitidas",v:rows.filter(r=>r.estado==="emitida").length,c:T.blue},
            {l:"Total facturado",v:`Q ${fmt(totalFac).split(".")[0]}`,c:T.purple},
            {l:"IVA acumulado",v:`Q ${fmt(totalIva).split(".")[0]}`,c:T.acc},
            {l:"Saldos pendientes",v:`Q ${fmt(totalSaldo).split(".")[0]}`,c:totalSaldo>0?T.sec:T.acc},
          ].map((s,i)=>(
            <div key={i} style={{background:T.surf,borderRadius:10,padding:14,textAlign:"center"}}>
              <div style={{fontSize:i>=2?13:22,fontWeight:800,color:s.c}}>{s.v}</div>
              <div style={{fontSize:11,color:T.sub,marginTop:2}}>{s.l}</div>
            </div>
          ))}
        </div>
        ...
        {/* Filtros */}
        <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
          {["todas","borrador","emitida","certificada","parcial","pagada","anulada"].map(f=>(
            <button key={f} onClick={()=>setFiltro(f)} style={{...S.btn(filtro===f?"primary":"ghost"),fontSize:11,padding:"5px 12px"}}>
        ...
        {loading?<Spinner/>:filtered.length===0?
          <div style={{...S.card,textAlign:"center",padding:36,color:T.sub}}>📭 Sin facturas</div>:(
          <div style={S.card}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr>{["Factura","Cliente / NIT","Servicio","Total","Anticipo","Saldo","Estado",""].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                {filtered.map(r=>{
                  const e=EST[r.estado]||EST.borrador;
                  const saldo=parseFloat(r.saldo_pendiente)||0;
                  const anticipo=parseFloat(r.anticipo_aplicado)||0;
                  return (
                    <tr key={r.id}>
                      <td style={S.td}>
                        <div style={{fontFamily:"monospace",fontSize:11,color:T.acc,fontWeight:700}}>{r.numero}</div>
                        <div style={{fontSize:10,color:T.mut}}>{r.serie} · {fmtD(r.fecha_emision)}</div>
                        {r.numero_autorizacion&&<div style={{fontSize:9,color:T.acc}}>✓ DTE cert.</div>}
                        {r.motivo_anulacion&&<div style={{fontSize:9,color:T.red}}>⚠ {r.motivo_anulacion.slice(0,30)}</div>}
                      </td>
                      <td style={S.td}>
                        <div style={{fontWeight:600,fontSize:12}}>{r.nombre_receptor}</div>
                        <div style={{fontSize:11,color:T.mut}}>NIT: {r.nit_receptor}</div>
                        {r.cliente_id&&<div style={{fontSize:10,color:T.acc}}>🔗 Vinculado</div>}
                      </td>
                      <td style={{...S.td,maxWidth:160}}>
                        <div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:155,fontSize:12}}>{r.descripcion_servicio}</div>
                        {r.reserva_id&&<div style={{fontSize:10,color:T.blue}}>📅 Vinculado a reserva</div>}
                        {r.cotizacion_id&&<div style={{fontSize:10,color:T.purple}}>📋 Vinculado a cotización</div>}
                      </td>
                      <td style={{...S.td,fontWeight:700,color:T.acc,whiteSpace:"nowrap"}}>Q {fmt(r.total)}</td>
                      <td style={{...S.td,color:anticipo>0?T.acc:T.mut,fontSize:12}}>
                        {anticipo>0?`Q ${fmt(anticipo)}`:"—"}
                      </td>
                      <td style={{...S.td,fontWeight:700,color:saldo>0?T.sec:T.acc,whiteSpace:"nowrap"}}>
                        {r.estado==="anulada"?"—":`Q ${fmt(saldo)}`}
                      </td>
                      <td style={S.td}><Badge color={e.c} bg={e.bg} label={e.l} small/></td>
                      <td style={S.td}>
                        <div style={{display:"flex",flexDirection:"column",gap:4,minWidth:120}}>
                          {r.estado==="emitida"&&<button onClick={()=>{setAuthFac(r);setAuthId("");}} style={{...S.btn("blue"),padding:"3px 8px",fontSize:10,width:"100%"}}>🔐 Certificar DTE</button>}
                          {["emitida","certificada","parcial"].includes(r.estado)&&<button onClick={()=>setModalPago(r)} style={{...S.btn("primary"),padding:"3px 8px",fontSize:10,width:"100%"}}>💰 Registrar pago</button>}
                          {r.estado!=="anulada"&&<button onClick={()=>{setEditItem(r);setVista("form");}} style={{...S.btn("ghost"),padding:"3px 8px",fontSize:10,width:"100%"}}>✏️ Editar</button>}
                          {!["anulada","pagada"].includes(r.estado)&&<button onClick={()=>setModalAnular(r)} style={{...S.btn("danger"),padding:"3px 8px",fontSize:10,width:"100%"}}>🚫 Anular</button>}
                        </div>
                      </td>
                    </tr>
                  );
            ...
            return(
              <div key={r.id} style={S.card}>
                ...
                <div style={{display:"flex",gap:5,paddingTop:10,borderTop:`1px solid ${T.bord}22`,flexWrap:"wrap"}}>
                  ...
                  <button onClick={()=>{window.open("mailto:?subject="+encodeURIComponent("Cotización "+r.numero)+"&body="+encodeURIComponent("Adjunto cotización "+r.numero+" por Q "+fmt(total)+".

Oscar Gálvez
502-31221538"));}} style={{...S.btn("ghost"),fontSize:11,padding:"4px 9px"}}>✉️</button>
