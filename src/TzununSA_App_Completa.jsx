...
function PageCotizaciones({showToast,empId}){
  ...
  if(vista==="form")return <div style={{padding:4}}><FormCotizacion initial={editItem} empId={empId} clientes={clientes} onSave={()=>{showToast("Guardada ✔");setEditItem(null);setVista("lista");load();}} onCancel={()=>{setEditItem(null);setVista("lista");}}/></div>;
  return(
    <div>
            ...
            return(
              <div key={r.id} style={S.card}>
                ...
                <div style={{display:"flex",gap:5,paddingTop:10,borderTop:`1px solid ${T.bord}22`,flexWrap:"wrap"}}>
                  ...
                  <button onClick={()=>{const doc=generarPDF({numero:r.numero,fecha:r.fecha_emision||today(),fecha_vence:r.fecha_vence,cliente:r.cliente_nombre,nit:r.cliente_nit,dir_cliente:r.cliente_dir,saludo:r.saludo,servicio:r.descripcion_servicio,caract:r.caract||["Vehículo","Aire acond.","Cinturones","Seguro"],incluidos:r.incluidos||["Combustible","Conductor","Atención"],beneficios:r.beneficios||["Viaje seguro","Puntualidad","Flexibilidad"],con_piloto:r.con_piloto!==false,sub:parseFloat(r.subtotal)||0,iva_pct:parseFloat(r.tasa_iva)||5,iva_amt:parseFloat(r.total_iva)||0,total_ef:total,total_tc:total*1.05,exch:parseFloat(r.tasa_cambio)||7.70,es_orden:r.orden_venta});if(doc)doc.save(r.numero+".pdf");}} style={{...S.btn("primary"),fontSize:11,padding:"4px 9px"}}>⬇ PDF</button>
                  <button onClick={()=>{const doc=generarPDF({numero:r.numero,fecha:r.fecha_emision||today(),fecha_vence:r.fecha_vence,cliente:r.cliente_nombre,nit:r.cliente_nit,dir_cliente:r.cliente_dir,saludo:r.saludo,servicio:r.descripcion_servicio,caract:r.caract||["Vehículo","Aire acond.","Cinturones","Seguro"],incluidos:r.incluidos||["Combustible","Conductor","Atención"],beneficios:r.beneficios||["Viaje seguro","Puntualidad","Flexibilidad"],con_piloto:r.con_piloto!==false,sub:parseFloat(r.subtotal)||0,iva_pct:parseFloat(r.tasa_iva)||5,iva_amt:parseFloat(r.total_iva)||0,total_ef:total,total_tc:total*1.05,exch:parseFloat(r.tasa_cambio)||7.70,es_orden:r.orden_venta});if(doc){const blob=doc.output("blob");const url=URL.createObjectURL(blob);const w=window.open(url);setTimeout(()=>w&&w.print(),1000);}}} style={{...S.btn("ghost"),fontSize:11,padding:"4px 9px"}}>🖨️</button>
                  <button onClick={()=>{const sub=encodeURIComponent("Cotización "+r.numero);const bod=encodeURIComponent("Adjunto cotización "+r.numero+" por Q "+fmt(total)+".

Oscar Gálvez
502-31221538");window.open("mailto:?subject="+sub+"&body="+bod);}} style={{...S.btn("ghost"),fontSize:11,padding:"4px 9px"}}>✉️</button>
...
function PageFacturacion({showToast,empId}){
  ...
  if(vista==="form")return <div style={{padding:4}}><FormFactura initial={editItem} empId={empId} clientes={clientes} reservas={reservas} cotizaciones={cotizaciones} anticipos={anticipos} onSave={()=>{showToast("Guardada ✔");setEditItem(null);setVista("lista");load();}} onCancel={()=>{setEditItem(null);setVista("lista");}}/></div>;
  return(
    <div>
...
function PageReportes({showToast}){
...
function PageGastos({showToast,empId}){
  ...
  return(
    <div>
      ...
      {tab==="proveedores"&&<ModProveedores empId={empId} showToast={(m,t)=>{showToast(m,t);reloadProv();}}/>}
    </div>
  );
}
...
export default function App(){
  ...
  const renderPage=()=>{
    ...
    if(pag==="reportes")     return <PageReportes showToast={showToast}/>;
    if(pag==="configuracion")return <PageConfiguracion showToast={showToast}/>;
    return <div style={{textAlign:"center",padding:60,color:T.sub}}>🚧 Módulo en construcción</div>;
  };
