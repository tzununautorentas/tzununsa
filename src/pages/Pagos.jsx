...
export default function PagePagos({ showToast, empId }) {
  ...
  const guardar = async () => {
    ...
    if (!f.cuenta_id) { showToast("Selecciona la cuenta bancaria donde se recibi├│ el pago", "err"); return; }
    ...
    try {
      ...
      showToast("Pago registrado Ô£ö"); setSaving(false); setShowForm(false); setF({ ...EMPTY }); load();
  ...
  const CAMPOS = [{ label: "Fecha", key: "fecha" }, { label: "Concepto", key: "concepto" }, { label: "Monto", key: "monto" }, { label: "M├®todo", key: "metodo" }, { label: "Referencia", key: "referencia" }];
  ...
  return (
    <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 11 }}>
            ...
            <Fld label="CUENTA BANCARIA *" span2>
              <select style={cuentas.length === 0 ? { ...S.sel, borderColor: T.red } : S.sel} value={f.cuenta_id} onChange={e => sf("cuenta_id", e.target.value)}>
                ...
                {cuentas.map(cu => <option key={cu.id} value={cu.id}>{cu.banco} ÔÇö {cu.numero_cuenta} ┬À Saldo: Q {fmt(cu.saldo_actual)}</option>)}
              </select>
              {cuentas.length === 0 && <div style={{ fontSize: 11, color: T.red, marginTop: 3 }}>ÔÜá No hay cuentas. Ve a La Banca para crear una.</div>}
            </Fld>
            ...
            <Fld label="VINCULAR A FACTURA (opcional)">
              <select style={S.sel} value={f.factura_id} onChange={e => sf("factura_id", e.target.value)}>
                ...
                {facturas.filter(fa => !["pagada", "anulada"].includes(fa.estado)).map(fa => <option key={fa.id} value={fa.id}>{fa.numero} ÔÇö {fa.nombre_receptor} ┬À Saldo: Q {fmt(fa.saldo_pendiente || fa.total)}</option>)}
              </select>
            </Fld>
            <Fld label="VINCULAR A RESERVA (opcional)">
              <select style={S.sel} value={f.reserva_id} onChange={e => sf("reserva_id", e.target.value)}>
                ...
                {reservas.filter(re => re.estado !== "cancelada").map(re => <option key={re.id} value={re.id}>{re.numero} ÔÇö {re.cliente_nombre} ┬À Saldo: Q {fmt(re.saldo || re.monto)}</option>)}
              </select>
            </Fld>
            <Fld label="CONCEPTO" span2><input style={S.inp} value={f.concepto} onChange={e => sf("concepto", e.target.value)} placeholder="Ej: Anticipo reserva Cob├ín..." /></Fld>
            ...
            <div style={{ display: "flex", alignItems: "flex-end" }}>
              <div style={{ display: "flex", gap: 8, width: "100%" }}>
                <button onClick={guardar} disabled={saving} style={{ ...S.btn("primary"), flex: 1, padding: 10 }}>{saving ? "Guardando..." : "­ƒÆ¥ Guardar"}</button>
      ...
      {loading ? <Spinner /> : rows.length === 0 ? <Empty icon="­ƒÆ░" msg="Sin pagos registrados" action="+ Registrar pago" onAction={() => setShowForm(true)} /> : (
        <div style={S.card}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr>{["Fecha", "Concepto", "M├®todo", "Referencia", "Monto", ""].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
            <tbody>
              {rows.map(r => (
                <tr key={r.id}>
                  ...
                  <td style={S.td}><button onClick={() => del(r.id)} style={{ ...S.btn("danger"), padding: "3px 8px", fontSize: 11 }}>­ƒùæ´©Å</button></td>
                </tr>
              ))}
            </tbody>
